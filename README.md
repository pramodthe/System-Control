...
🎉 Setup verification complete! You're ready to run the application.
```

### 4. Grant macOS Permissions (First Run)

The app needs two permissions:

**Accessibility** (for mouse/keyboard control):
1. Open **System Settings** → **Privacy & Security** → **Accessibility**
2. Add and enable **Terminal** (or your IDE)
3. Restart Terminal
...
Start speaking! Try: "What's on my screen?" or "Move the mouse to the center"
...
- **Solution**: Create `.env` file with `echo "GOOGLE_API_KEY=your-key" > .env`

**Problem**: `GOOGLE_API_KEY is not configured`
- **Solution**: Get API key from [Google AI Studio](https://aistudio.google.com/app/apikey) and add to `.env`
...
**Problem**: Commands detected but no action
- **Solution**: Grant Accessibility permission (see step 4 above)
- **Test**: Run `uv run python test_permissions.py`

### ❌ Screen capture fails

**Problem**: `CoreGraphics.CGWindowListCreateImage() failed`
- **Solution**: Grant Screen Recording permission (see step 4 above)

### ❌ Module not found errors

**Problem**: `ModuleNotFoundError: No module named 'pynput'`
- **Solution**: Always run with `uv run python test3.py` (not just `python test3.py`)
- **Alternative**: Run `uv sync` to ensure all dependencies are installed

### ❌ Voice not responding

**Problem**: Tools execute but AI doesn't speak back
- **Solution**: Use headphones to prevent audio feedback
- **Check**: Ensure microphone and speakers are working

### ❌ Invalid API key

**Problem**: `401 Unauthorized` or API errors
- **Solution**: Verify your API key is correct in `.env` file
- **Check**: Make sure there are no extra spaces or quotes around the key
